...
**Hoy:** convertir series temporales en problemas supervisados, aplicar Random Forest / XGBoost / LightGBM con `skforecast`, y aprender a evaluar bien.

. . .

- Teoría densa: **3 h** (cinco bloques).
- Práctica: **1 h 30 min** — features, baseline, RF/XGBoost, backtesting.
...
- **Fuga de información** es el enemigo silencioso.
...
```{mermaid}
flowchart LR
  L[Linear<br/>Ridge / Lasso] --> RF[Random Forest]
  RF --> XGB[XGBoost]
  XGB --> LGB[LightGBM]
  XGB --> CB[CatBoost]
  L -.-> BL[(baselines)]
...
- **Bosques aleatorios**: baja varianza vía bagging, no extrapolan.
...
Tres baselines que un modelo debe **batir** para ser publicable:
...
**Por qué lineal sigue siendo relevante:**

- Cuando hay 50 lags + rolling + calendario, `LinearRegression` plana se sobreajusta. Ridge no.
- Es la baseline justa contra la que comparar el resto.
- En cuencas reguladas, captura el 60-80% de la varianza.
...
- Documentación nativa en español (cienciadedatos.net).
...
# Bloque 3 · Predicción multi-paso
...
- **Idea:** entrenar un solo modelo de 1 paso. Re-aplicarlo $h$ veces, alimentando sus propias predicciones.
- **Pros:** un solo modelo; aprovecha la estructura autorregresiva.
- **Contras:** **acumulación de error** — pequeños errores de paso 1 se amplifican en paso $h$.
...
```{mermaid}
flowchart LR
  M1[Modelo h=1] --> P1[ŷ_{t+1}]
  M2[Modelo h=2] --> P2[ŷ_{t+2}]
  M3[Modelo h=3] --> P3[ŷ_{t+3}]
```
...
# Bloque 5 · Métricas hidrológicas
...
**Rangos típicos** (Moriasi et al., 2015):
...
**Ventaja:** descompone el error en **tres causas** independientes. Permite diagnosticar:
...
Negativo = sobreestima. Positivo = infraestima.

Crítico para balance hídrico anual.
...
```python
# Ejemplo conformal con skforecast
